...
@app.command("get")
def get_ticket(ticket_id: str, json_output: bool = False) -> None:
    out = _svc().get(ticket_id)
    rprint(json.dumps(out.model_dump(), indent=2) if json_output else out.model_dump())


@app.command("create")
def create_ticket(result_id: str, comment: str = "", json_output: bool = False) -> None:
...
@app.command("update")
def update_ticket(
    ticket_id: str, status: str, comment: str = "", json_output: bool = False
) -> None:
