...
@app.command("get")
def get_override(override_id: str, json_output: bool = False) -> None:
    out = _svc().get(override_id)
    rprint(json.dumps(out.model_dump(), indent=2) if json_output else out.model_dump())


@app.command("create")
def create_override(text: str, nvt_oid: str = "", json_output: bool = False) -> None:
    out = _svc().create(text_value=text, nvt_oid=nvt_oid)
    rprint(json.dumps(out.model_dump(), indent=2) if json_output else out.model_dump())


@app.command("update")
def update_override(override_id: str, text: str, json_output: bool = False) -> None:
